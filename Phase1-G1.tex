...
\begin{algorithm}[H]  % toujours mettre le H
    ...
    \Repeat{$PercentReached \leq PercentInfo$ or $niter < MaxIter$}{
        ...
        $V = gram-schmidt(V)\footnote{Nous ne rappellons pas l'algorithme d'orthogonalisation de Gram-Schmidt, mais celui-çi est implémenté dans le code}$\;
        $H = Z^T*V$\;
        $H = H^T*H$\;
        ...
        $V = V*X$
        \For {$i=converged + 1, n$}{
            \eIf{$\frac{\norme{Z*Z^T*V(i) - A(i,i).V(i)}}{normeA}$}{
                $converged = converged + 1$\;
                $PercentReached = 1 - \frac{\norme{Z - \sum\limits{j=1}^i \sqrt{A(i,i)}V(i)V(i)^TZ^T}}{\sqrt{normeA}}$\;
            } {
                break\;
            }
        }
